...
Singleton {
    id: root
    property string from: Config.options?.hyprsunset?.from ?? "19:00" 
    property string to: Config.options?.hyprsunset?.to ?? "06:30"
    property bool automatic: Config.options?.hyprsunset?.automatic && (Config?.ready ?? true)
    property int colorTemperature: Config.options?.hyprsunset?.colorTemperature ?? 5000
