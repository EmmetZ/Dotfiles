...
MouseArea { // Notification group area
    ...
    onContainsMouseChanged: {
        ...
        if (root.containsMouse) {
            ...
            root.notifications.forEach(notif => {
                Notifications.resetTimeout(notif.notificationId, 3000);
        ...
        RowLayout { // Left column for icon, right column for content
            ...
            ColumnLayout {
                ...
                Item { // App name (or summary when there's only 1 notif) and time
                    ...
                    // spacing: 0
                    Layout.fillWidth: true
                    property real fontSize: Appearance.font.pixelSize.medium
                    ...
                    RowLayout {
                        ...
                        StyledText {
                            ...
                            Layout.fillWidth: true
                            text: (topRow.showAppName ? notificationGroup?.appName :
                                    (
                                        (notificationGroup?.notifications[0]?.urgency == NotificationUrgency.Critical ? "󰀦 " : "") +
                                        notificationGroup?.notifications[0]?.summary
                                    )
                                ) || ""
                            font.bold: true
                            font.pixelSize: topRow.showAppName ? topRow.fontSize : Appearance.font.pixelSize.normal
