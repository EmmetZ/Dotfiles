...
Singleton {
    ...
    property QtObject mocha
    property QtObject rounding
    property QtObject font
    property QtObject sizes
    property QtObject bar
    property QtObject media
    ...
    ColorQuantizer {
        id: wallColorQuant
        source: Qt.resolvedUrl(root.background.path)
        depth: 0 // 2^0 = 1 color
        rescaleSize: 10
    }
    ...
    property real workspaceTransparency: 0.1

    FileView {
        id: wallpaperPathFile
        path: Qt.resolvedUrl(Directories.wallpaperPath)
        watchChanges: true
        onFileChanged: {
            this.reload();
        }
        onLoadedChanged: {
            root.background.path = wallpaperPathFile.text().trim();
        }
        onTextChanged: {
            root.background.path = wallpaperPathFile.text().trim();
        }
    ...
    colors: QtObject {
        ...
        property QtObject bar: QtObject {
            property color background: ColorUtils.applyAlpha(mocha.surface0, 1 - backgroundTransparency)
            property color popupBackground: ColorUtils.applyAlpha(mocha.surface0, 1 - backgroundTransparency)
            property color border: mocha.surface1
            property color text: mocha.text
            property color activeBorder: mocha.lavender
        }
    }
    ...
    font: QtObject {
        property QtObject family: QtObject {
            property string monospace: "Arimo Nerd Font"
            property string iconFont: "FiraCode Nerd Font"
    ...
    sizes: QtObject {
        ...
        property real osdWidth: 30
        property real osdHeight: 300
        property real controlCenterWidth: 400
        property real timeMenuWidth: 440
    }

    bar: QtObject {
        property int height: 24
        property int baseVerticalBarWidth: 40
        property int radius: 6
        property int gap: 18
        property int verticalBarWidth: 24
        property bool enableShadow: false
        property int marginTop: enableShadow ? 0 : 3
        property int marginLeft: enableShadow ? 0 : 4
        property int marginRight: enableShadow ? 0 : 4
        property int marginBottom: enableShadow ? 0 : 3
    }

    media: QtObject {
        property real controlsWidth: 440
        property real controlsHeight: 160
    }

    function reloadTheme() {
        if (Config.options.appearance.palette === "auto") {
            Quickshell.execDetached(["matugen", "image", `${root.background.path}`, "-m", `${root.m3colors.darkmode ? "dark" : "light"}`]);
        } else {
            Quickshell.execDetached(["matugen", "image", `${root.background.path}`, "-t", Config.options.appearance.palette, "-m", `${root.m3colors.darkmode ? "dark" : "light"}`]);
        }
    }
