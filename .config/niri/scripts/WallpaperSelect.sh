...
# variables
focused_monitor=$(niri msg focused-output | head -1 | sed -E 's/.*\(([^)]+)\).*/\1/')
...
main() {
  ...
  if [[ $pic_index -ne -1 ]]; then
    ...
    swww img -o "$focused_monitor" "${PICS[$pic_index]}" -n overview --transition-fps $FPS --transition-type fade --transition-duration $DURATION
